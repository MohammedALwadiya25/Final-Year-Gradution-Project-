...
export function getConfig(): WazuhConfig {
  ...
  const password = process.env.WAZUH_PASSWORD || process.env.WAZUH_PASS;
  if (!password) {
    throw new Error(
      "WAZUH_PASSWORD environment variable is required. Set it to your Wazuh API password. WAZUH_PASS is also accepted as a guide-compatible alias."
    );
  }
