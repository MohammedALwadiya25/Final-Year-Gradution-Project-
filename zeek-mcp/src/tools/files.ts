...
export function registerFileTools(server: McpServer, config: ZeekConfig): void {
  server.tool(
    "zeek_query_files",
    "Search Zeek file extraction logs. Filter by MIME type, filename, hash values, source IP, and file size.",
    {
      mimeType: z
        .string()
        .optional()
        .describe("MIME type (application/x-dosexec, application/pdf, etc.)"),
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `Error querying files: ${error instanceof Error ? error.message : String(error)}`,
            },
          ],
          isError: true,
        };
