...
export function registerSslTools(server: McpServer, config: ZeekConfig): void {
  server.tool(
    "zeek_query_ssl",
    "Search Zeek SSL/TLS connection logs. Filter by SNI hostname, TLS version, certificate validation status, subject, and issuer.",
    {
      serverName: z
        .string()
        .optional()
        .describe("SNI hostname (supports wildcards)"),
      srcIp: z.string().optional().describe("Source IP address"),
      dstIp: z.string().optional().describe("Destination IP address"),
      version: z
        .string()
        .optional()
        .describe("TLS version (TLSv10, TLSv11, TLSv12, TLSv13, SSLv3)"),
      validationStatus: z
        .string()
        .optional()
        .describe(
          "Certificate validation status (ok, self signed certificate, etc.)",
        ),
      subject: z
        .string()
        .optional()
        .describe("Certificate subject (partial match)"),
      issuer: z
        .string()
        .optional()
        .describe("Certificate issuer (partial match)"),
    ...
    async (params) => {
      try {
        ...
        if (params.srcIp) {
          filters.push({
            field: "id.orig_h",
            op: params.srcIp.includes("/") ? "cidr" : "eq",
            value: params.srcIp,
          });
        }
        if (params.dstIp) {
          filters.push({
            field: "id.resp_h",
            op: params.dstIp.includes("/") ? "cidr" : "eq",
            value: params.dstIp,
          });
        }
        if (params.version) {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `Error querying SSL: ${error instanceof Error ? error.message : String(error)}`,
            },
          ],
          isError: true,
        };
...
function formatSsl(record: Record<string, unknown>): Record<string, unknown> {
  return {
    timestamp: record.ts
      ? new Date((record.ts as number) * 1000).toISOString()
      : undefined,
    uid: record.uid,
