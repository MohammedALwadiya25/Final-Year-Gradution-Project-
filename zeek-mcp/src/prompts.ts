...
export function registerPrompts(server: McpServer): void {
  server.prompt(
    ...
    "Triage a Suricata alert by cross-referencing with Zeek logs for full context. Determines if the alert is a true positive, false positive, or needs escalation.",
    {
      ...
      srcIp: z.string().optional().describe("Source IP from the alert"),
    },
    ({ signatureId, srcIp }) => ({
      messages: [
        {
  ...
  server.prompt(
    ...
    "Full host investigation workflow - cross-log analysis, anomaly identification, and connection profiling for a specific IP address.",
    {
      ...
      timeWindow: z
        ...
        .optional()
        .describe(
          "Time window to investigate (e.g. 'last 24h', '2024-01-15 to 2024-01-16')",
        ),
    },
    ({ ip, timeWindow }) => ({
      messages: [
...
5. **SSL/TLS anomalies**: Run zeek_query_ssl filtering by validation_status
