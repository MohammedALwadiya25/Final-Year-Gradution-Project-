...
export function registerPrompts(server: McpServer): void {
  server.prompt(
    ...
    "Alert investigation workflow - query the alert, find related flows, check protocol details",
    {
      signatureId: z
        .string()
        .optional()
        .describe("Suricata rule SID to investigate"),
    ...
    async (args) => {
      ...
      const srcPart = args.srcIp ? ` Focus on source IP ${args.srcIp}.` : "";
        ...
        messages: [
          {
            ...
            content: {
              ...
              text: [
                ...
                "7. Assess the severity and recommend a response:",
  ...
  server.prompt(
    ...
    "Proactive threat hunting across Suricata data",
    {
      timeWindow: z
        .string()
        .optional()
        .describe("Time window to hunt in (e.g., '24h', '7d')"),
      focus: z
        .string()
        .optional()
        .describe(
          "Specific focus area (e.g., 'beaconing', 'exfiltration', 'lateral')",
        ),
    },
    async (args) => {
  ...
  server.prompt(
    "incident-response",
    "Full incident response workflow using Suricata alerts and Zeek enrichment",
    {
      alertSid: z
        .string()
        .optional()
        .describe("Suricata alert SID to investigate"),
        ...
        messages: [
          {
            ...
            content: {
              ...
              text: [
                ...
                "9. Check suricata_dga_detection for domain generation algorithm indicators.",
                "10. Run suricata_beaconing_detection for C2 communication patterns.",
                "11. Run suricata_exfiltration_detection for data theft indicators.",
                "",
                "**Phase 4: Containment Recommendations**",
                "12. Based on findings, recommend containment actions:",
                "    - Block malicious IPs/domains",
                "    - Isolate affected hosts",
                "    - Deploy Suricata rules to detect variants",
  ...
  server.prompt(
    ...
    "Generate a network baseline report to establish normal traffic patterns",
    {
      timeWindow: z
        .string()
        .optional()
        .describe("Time window for baseline (e.g., '24h', '7d')"),
    },
    async (args) => {
      const timePart = args.timeWindow
